...
import type { Function, BusinessDomain } from '../../types/api';

const FunctionList = () => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [dataSource, setDataSource] = useState<Function[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await functionApi.list(params);
      if (response.code === 200) {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button
            ...
            size="small"
            icon={<EditOutlined />}
            onClick={() => navigate(`/functions/${record.id}/edit`)}
          >
            编辑
          </Button>
          <Popconfirm
  ...
  return (
    <div>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ margin: 0 }}>函数管理</h2>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/functions/new')}>
          新建函数
        </Button>
      </div>

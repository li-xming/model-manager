...
import { interfaceApi, businessDomainApi } from '../../api';
import type { Interface, BusinessDomain } from '../../types/api';
...
const InterfaceList = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await interfaceApi.list(params);
      if (response.code === 200) {
  ...
  const columns: ColumnsType<Interface> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button
            ...
            size="small"
            icon={<EditOutlined />}
            onClick={() => navigate(`/interfaces/${record.id}/edit`)}
          >
            编辑
          </Button>
          <Popconfirm
  ...
  return (
    <div>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/interfaces/new')}>
          新建接口
        </Button>
      </div>
